...
public class JDBCProjectEx3
	...
	//private String sqlUpdate = "UPDATE CUSTOMERS ";
	...
	public void update() {
		...
		if(strNo.length() < 1) {
			JOptionPane.showMessageDialog(null, "번호는 필수입니다. 입력하세요.");
			return;
		}
	...
	public void search() {
		String strName = txtName.getText();
		if(strName.length() < 1) {
			JOptionPane.showMessageDialog(null, "이름은 필수입니다. 입력하세요.");
			return;
		}
		try {
	...
	private void total() {  // 전체보기(select all) 버튼 이벤트 처리 함수
		try {
			...
			if( model == null ) model = new MyModel();  // 객체생성, 메모리에 할당, 생성자함수 자동호출
